  ...
  if (!token) {
    return (
      <div className="text-center">
        ...
        <h2 className="mb-2 font-display text-2xl text-[#2D3047]">Lien invalide</h2>
        <p className="mb-6 text-sm text-[#5C5F77]">Ce lien de réinitialisation est invalide ou a expiré.</p>
        <Link href="/auth/forgot-password" className="inline-flex items-center gap-2 text-sm font-semibold text-[#E07A5F] transition-colors hover:text-[#C96040]">Demander un nouveau lien</Link>
      </div>
    );
  }
  ...
  if (success) {
    return (
      <div className="text-center">
        ...
        <h2 className="mb-2 font-display text-2xl text-[#2D3047]">Mot de passe réinitialisé !</h2>
        <p className="mb-6 text-sm text-[#5C5F77]">Votre mot de passe a été modifié avec succès. Vous pouvez maintenant vous connecter avec votre nouveau mot de passe.</p>
        <Link href="/auth/signin" className="inline-flex items-center gap-2 rounded-2xl bg-gradient-to-r from-[#E07A5F] to-[#F4A261] px-6 py-3 text-sm font-bold text-white transition-all duration-300 hover:translate-y-[-1px] hover:shadow-[0_12px_30px_rgba(224,122,95,0.35)]">
  ...
  return (
    <>
      <div className="mb-6 text-center">
        ...
        <h2 className="mb-1 font-display text-2xl text-[#2D3047]">Nouveau mot de passe</h2>
        <p className="text-sm text-[#5C5F77]">Choisissez un nouveau mot de passe sécurisé.</p>
      </div>

      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        <div>
          <label htmlFor="password" className="mb-1.5 block text-xs font-semibold uppercase tracking-[0.2em] text-[#8E90A6]">Nouveau mot de passe</label>
        ...
        <div>
          <label htmlFor="confirmPassword" className="mb-1.5 block text-xs font-semibold uppercase tracking-[0.2em] text-[#8E90A6]">Confirmer le mot de passe</label>
          <div className="relative">
            <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#8E90A6]">
              <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" /></svg>
            </span>
            <input id="confirmPassword" name="confirmPassword" type={showPassword ? "text" : "password"} required minLength={6} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="••••••••" className="w-full rounded-xl border border-[#2D3047]/10 bg-white py-3 pr-4 pl-10 text-sm text-[#2D3047] placeholder:text-[#8E90A6] outline-none transition focus:border-[#E07A5F] focus:ring-2 focus:ring-[#E07A5F]/20" />
          </div>
        </div>
    ...
    <div className="relative min-h-[calc(100vh-4rem)] overflow-hidden bg-[#FFFBF5] px-4 py-10 sm:px-6">
      ...
      <div className="relative z-10 mx-auto w-full max-w-md animate-fade-up">
        <div className="mb-8 text-center">
          ...
          <p className="mt-3 text-sm text-[#5C5F77]">Réinitialisation du mot de passe</p>
        </div>

        <div className="glass card-shadow rounded-3xl border border-[#E07A5F]/10 p-6 sm:p-8">
          <Suspense fallback={
            <div className="text-center py-8">
              <svg className="mx-auto h-8 w-8 animate-spin text-[#E07A5F]" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>
            </div>
          }>
            <ResetPasswordForm />
          </Suspense>
