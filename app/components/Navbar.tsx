    ...
    >
      <div className="container mx-auto px-4 h-20 flex items-center justify-between">
        ...
        <div className="flex items-center gap-8 font-medium text-gray-600">
          <NavLink href="/" currentPath={pathname}>Accueil</NavLink>
          <NavLink href="/about" currentPath={pathname}>À propos</NavLink>
...
const NavLink = ({ href, currentPath, children }: { href: string, currentPath: string, children: React.ReactNode }) => {
  ...
  return (
    <Link href={href} className="relative group py-2">
      <span className={`transition-colors duration-300 ${isActive ? 'text-blue-600 font-bold' : 'group-hover:text-blue-600'}`}>
        {children}
      </span>
      {isActive && (
        <motion.div 
